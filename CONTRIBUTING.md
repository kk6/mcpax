...
グローバルな act 設定で `--bind` オプションを使用している場合、ワークフロー内で作成される仮想環境がホストの `.venv` を上書きする可能性があります。

本プロジェクトの CI ワークフローでは、この問題を回避するため `UV_PROJECT_ENVIRONMENT=/tmp/.venv` を設定し、コンテナ内の仮想環境をホストと分離しています。
