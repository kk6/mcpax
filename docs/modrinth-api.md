...
`core/cache.py` の `ApiCache` が JSON ファイルベースの TTL キャッシュを提供しています。

| データ種別 | デフォルト TTL |
|---|---|
| プロジェクト情報 | 設定値（`ApiCache` 初期化時に指定。デフォルト 300 秒） |
| バージョン一覧 | 同上 |

キャッシュは `context manager` として使用し、`flush()` または `__aexit__` でディスクへ書き出します。
